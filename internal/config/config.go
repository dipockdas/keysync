...
type RepoConfig struct {
	...
	// Secrets is an optional allowlist: when non-empty, only these key names are pushed.
	Secrets []string `json:"secrets,omitempty"`
	// Exclude lists keys that must never be pushed (local-only credentials, obsolete keys).

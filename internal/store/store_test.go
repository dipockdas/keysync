...
func TestServiceName(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := serviceName(tt.scope, tt.project)
			if got != tt.want {
				t.Errorf("serviceName(%q, %q) = %q, want %q", tt.scope, tt.project, got, tt.want)
			}
		})
	}
}
...
func TestParseServiceName(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotScope, gotProj := parseServiceName(tt.input)
			if gotScope != tt.wantScope || gotProj != tt.wantProj {
				t.Errorf("parseServiceName(%q) = (%q, %q), want (%q, %q)",
					tt.input, gotScope, gotProj, tt.wantScope, tt.wantProj)
			}
		})
	}
}
...
func TestMemKey(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := memKey(tt.scope, tt.project, tt.key)
			if got != tt.want {
				t.Errorf("memKey(%q, %q, %q) = %q, want %q", tt.scope, tt.project, tt.key, got, tt.want)
			}
		})
	}
}
...
func TestMemoryStore_List(t *testing.T) {
	...
	for _, e := range entries {
		m.Set(ctx, e.scope, e.project, e.key, "val")
	}
	...
	tests := []struct {
		...
		{"project app-a", ScopeProject, "app-a", 2},
		{"project app-b", ScopeProject, "app-b", 1},
		{"nonexistent project", ScopeProject, "other", 0},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := m.List(ctx, tt.scope, tt.project)
			if err != nil {
...
func TestMemoryStore_ListSorted(t *testing.T) {
	...
	expected := []SecretEntry{
		{Scope: ScopeGlobal, Project: "", Key: "A_KEY"},
		{Scope: ScopeGlobal, Project: "", Key: "Z_KEY"},
		{Scope: ScopeProject, Project: "a-app", Key: "B_KEY"},
		{Scope: ScopeProject, Project: "z-app", Key: "A_KEY"},
	}

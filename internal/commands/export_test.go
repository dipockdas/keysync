...
func TestExportCmd_EnvOverridesProject(t *testing.T) {
	...
	cmd := newExportCmd()
	stdout, stderr, err := captureCommand(cmd, []string{"--env", "staging"})
	if err != nil {
		t.Fatalf("RunE failed: %v", err)
	}
	if stderr != "" {
...
func TestExportCmd_SingleKey(t *testing.T) {
	defer setupTest(t)()
	ctx := context.Background()
	secretSt.Set(ctx, store.ScopeGlobal, "", "", "API_KEY", "api-val")
	secretSt.Set(ctx, store.ScopeGlobal, "", "", "OTHER_KEY", "other-val")

	cmd := newExportCmd()
	stdout, stderr, err := captureCommand(cmd, []string{"API_KEY"})
	if err != nil {
		t.Fatalf("RunE failed: %v", err)
	}
	if stderr != "" {
	...
	if !strings.Contains(stdout, "export API_KEY=") {
		t.Errorf("stdout missing API_KEY: %s", stdout)
	}
	if strings.Contains(stdout, "OTHER_KEY") {
		t.Errorf("stdout should not export OTHER_KEY: %s", stdout)
	}
}
